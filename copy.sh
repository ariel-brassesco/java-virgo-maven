...
if [ $target_branch == $origin_branch ]; then   
  ...
  if [ $? -eq 0 ]; then
    ...
    /bin/echo -e "\e[1;35mDownloading branch $target_branch for $name.\e[0m"
  else
    download_url=`echo $clone_url | sed 's/\.git/\/archive\/'$default_branch'.zip/g'`
    /bin/echo -e "\e[1;35mDownloading branch $default_branch for $name.\e[0m"
  fi
